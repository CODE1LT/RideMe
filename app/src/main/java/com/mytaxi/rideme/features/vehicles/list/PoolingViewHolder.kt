...
class PoolingViewHolder(taxiView: View,
    ...
    internal inner class AddressResultReceiver(handler: Handler) : ResultReceiver(handler) {
        override fun onReceiveResult(resultCode: Int, resultData: Bundle?) {
            tvAddress.text = resultData?.getString(FetchAddressIntentService.RESULT_DATA_KEY) ?: "-"
        }
    }
    ...
    private fun startIntentService(lastLocation: Location) {
        val intent = Intent(context, FetchAddressIntentService::class.java).apply {
            ...
            putExtra(FetchAddressIntentService.LOCATION_DATA_EXTRA, lastLocation)
        }
